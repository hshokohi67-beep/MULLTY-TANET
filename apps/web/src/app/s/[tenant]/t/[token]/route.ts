...
import { storeUrl, TENANT_SLUG } from '@/lib/storefront';
...
export async function GET(request: NextRequest, { params }: RouteContext<'/s/[tenant]/t/[token]'>) {
  ...
  if (!TENANT_SLUG.test(tenant) || token.length < 20 || token.length > 120) {
    return NextResponse.redirect(storeUrl(tenant, '?qr=invalid'), 303);
  }
  ...
  const response = NextResponse.redirect(storeUrl(tenant, `?branch=${encodeURIComponent(joined.branch.slug)}`), 303);
  const cookie = { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax' as const, path: `/s/${tenant}`, maxAge: 4 * 3600 };
  ...
  response.cookies.set('cs_cart', '', { path: `/s/${tenant}`, maxAge: 0 });
