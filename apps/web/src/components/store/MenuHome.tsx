...
import { StoreHeroArt } from './StoreArt';
import { getMenu, getStories, storeUrl } from '@/lib/storefront';
...
export function pickBranch(branches: StoreBranch[], slug: unknown): StoreBranch | undefined {
...
/**
 * The menu home: which branch, open or closed, and the full menu (cached HTML; the cart loads on
 * the client). Shown at /menu, and at the café's root while it has no published landing page.
 */
export async function MenuHome({ tenant, store, query }: { tenant: string; store: Storefront; query: Record<string, string | string[] | undefined> }) {
        ...
        {store.branches.length > 1 ? (
          <nav aria-label="انتخاب شعبه" className="no-scrollbar mt-4 flex gap-2 overflow-x-auto">
            {store.branches.map((b) => (
              <Link key={b.id} href={b === store.branches[0] ? `/s/${tenant}/menu` : `/s/${tenant}/menu?branch=${encodeURIComponent(b.slug)}`}
