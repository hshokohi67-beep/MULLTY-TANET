...
- Marketplace (`/explore`) reads only the `marketplace_stores` projection, built by `Marketplace\Actions\ProjectStore` from an explicit allow-list and serialised by `StorePresenter` key lists. New public facts go through both; a model whose change affects them must be added to the `MarketplaceSync` events in `MarketplaceServiceProvider`. Never expose `tenant_id`, internal ids or anything the café didn't publish.
