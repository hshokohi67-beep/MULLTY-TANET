...
- SMS has two lines. The platform `SmsProvider` (Raygan) sends only login codes and platform messages. Everything a café sends (order updates, birthdays, reports, campaigns) goes through `CafeMessenger`/`Messaging\Actions\SendCafeSms` on the café's own panel. Campaigns go only to opted-in customers, within quiet hours and the daily cap, with the «لغو۱۱» footer.
...
- Café subdomains: `{slug}.{NEXT_PUBLIC_STORE_BASE_DOMAIN}` is rewritten to `/s/{slug}` in `proxy.ts`. Build storefront links with `lib/store-links` (`storeLink`/`storeHref`), never by hand. Cookie scope comes from the Host (`slugFromHost`), not proxy headers. API-built storefront URLs go through `StorefrontUrl`.
